...
const replyWithTemplate =
  (intercomPayload: any = {}) =>
  (templates: Array<any>, recipientId: string) =>
    Promise.all(
      templates
...
export default async function intercomWebhookController(
  ...
  const conversation =
    intercomPayload.data.item.conversation_parts.conversation_parts.pop() as ConversationPart;
  ...
  const [_, response] = await Promise.all([
    ...
    handleAsBot({ intercomPayload, chatPlatform, customer, conversation }),
  ]);
