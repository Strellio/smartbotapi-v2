...
import { MESSAGE_MEDIA_TYPE, MESSAGE_TYPE } from "../../../../../../models/messages/schema";
...
export default function formatAndSaveMessage({ customer, isCustomerMessage, isChatWithLiveAgent, chatPlatform, text, media, customGenericTemplate = [], externalId }: {
    text?: string
    customer: any
    media?: MessageMedia[]
    ...
    return conversationsService().create({
        ...
        media,
        type: media?.length ? MESSAGE_TYPE.MEDIA : customGenericTemplate.length ? MESSAGE_TYPE.GENERIC_TEMPLATE : MESSAGE_TYPE.TEXT,
        text,
