...
export default function intercomLib() {
  return {
    getAuthRedirect: (businessId: string = required("businessId")) =>
      `${INTERCOM_BASE_URL}/a/oauth/connect?client_id=${config.INTERCOM_CLIENT_ID}&state=${businessId}`,
    geToken: (code: string = required("code")): Promise<Auth> =>
      request
        .post(`${INTERCOM_BASE_URL}/auth/eagle/token`, {
          code,
          client_id: config.INTERCOM_CLIENT_ID,
          client_secret: config.INTERCOM_CLIENT_SECRET,
        })
    ...
    conversations: {
      create: ({
        ...
        request.post(
          `${INTERCOM_BASE_URL}/conversations/${conversationId}/reply`,
          {
